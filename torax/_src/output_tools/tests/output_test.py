...
class StateHistoryTest(parameterized.TestCase):
  ...
  def test_turbulent_transport_is_saved(self):
    """Tests that the turbulent transport per-model child node is saved."""
    output_xr = self.history.simulation_output_to_xr()
    self.assertIn(output_keys.TURBULENT_TRANSPORT, output_xr.children)
    turbulent_tree = output_xr.children[output_keys.TURBULENT_TRANSPORT]
    self.assertIn('prescribed', turbulent_tree.children)
    prescribed_ds = turbulent_tree.children['prescribed'].dataset
